...
const char *WINDOWTITLE = { "OpenGL / Project 3 -- Bradley Martin" };
...
GLuint	TorusList;				// list to hold wire torus
GLuint	BladeList;				// list to hold Helicopter blades
GLuint	HelicopterList;			// List to hold Helicopter
GLuint	Tex0, Tex1;				// Texture variables
...
float	BladeAngle;				// incremeter for blade animation
int		WhichColor;				// index into Colors[ ]
int		WhichProjection;		// ORTHO or PERSP
int		WhichPerspective;		// outside or inside
int		WhichTexture;			// decides what texture
...
Display()
{
	...
	if(WhichPerspective == INSIDE) {
		gluLookAt(-.4, 1.8, -4.9,   0., 0., -12.,   0., 0., -1.);
	}
	else {

		gluLookAt(8., 3., 12., 0., 0., 0., 0., 1., 0.);
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Project 3" );
...
InitMenus( )
{
	...
	int permenu = glutCreateMenu(DoPerMenu);
	...
	glutAddMenuEntry("Replace", REPLACE);
	glutAddMenuEntry("Distortion", DISTORTION);
	...
	glutAddSubMenu("Perspective", permenu);
...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			p->ny = y;
			p->nz = z;
			if (WhichTexture == DISTORTION) {
				p->s = (lng + M_PI) / (2. * M_PI);
				p->t = ((lat + M_PI / 2.) / M_PI) + cos(2. * (lat + M_PI / 2. + Time)) * 0.30;
			}
			else {
				p->s = (lng + M_PI) / (2. * M_PI);
				p->t = (lat + M_PI / 2.) / M_PI;
